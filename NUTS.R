...
n <- 250
a <- 2
A <- matrix(0,nrow=250,ncol=250)
for(i in 1:250){
  A[i,] <- rnorm(250)+a*rnorm(1)
}
A <- A%*%t(A)
...
set.seed(5)
library(mvtnorm)
data <- rmvnorm(50,mean=rep(0,250),sigma=C)
...
f <- function(theta){
  ...
  C.inv <- solve(C)
  for(i in 1:50){
    val <- val+t(data[i,]-theta)%*%C.inv%*%(data[i,]-theta)
  }
  val <- val+t(theta)%*%theta
  val <- -0.5*val
  val <- as.vector(val)
  return(val)
}
...
theta=rep(0,250)
tmp <- NUTS(theta, f, grad_f, n_iter=50, M_diag = NULL, M_adapt = 50, delta = 0.5, max_treedepth = 30, eps = 1, verbose = TRUE)
out <- capture.output(summary(tmp))
cat("NUTS1", out, file="summary_NUTS1.txt", sep="n", append=TRUE)
